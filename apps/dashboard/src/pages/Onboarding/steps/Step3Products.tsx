 ...
 * File    : src/pages/Onboarding/steps/Step3Products.tsx
 * Role    : First product creation form. POSTs to /api/v1/products.
 */
...
export function Step3Products({ onNext, onBack }: Props) {
  ...
  async function handleNext() {
    if (!form.name.trim() || !form.priceIdr || !form.weightGrams) {
      setError('Name, price, and weight are required.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      await api.post('/products', {
        ...
        weightGrams: parseInt(form.weightGrams, 10),
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={form.name} onChange={e => set('name', e.target.value)} placeholder="Buku Python Pro" className={inputCls} />
          </div>
          <div>
            <label className="block text-sm text-slate-300 mb-1">SKU</label>
            <input type="text" value={form.sku} onChange={e => set('sku', e.target.value)} placeholder="PYTH-001" className={inputCls} />
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-slate-300 mb-1">Price (IDR) *</label>
            <input type="number" value={form.priceIdr} onChange={e => set('priceIdr', e.target.value)} placeholder="150000" className={inputCls} />
          </div>
          <div>
            <label className="block text-sm text-slate-300 mb-1">Weight (grams) *</label>
            <input type="number" value={form.weightGrams} onChange={e => set('weightGrams', e.target.value)} placeholder="300" className={inputCls} />
          </div>
        </div>
        ...
        <div>
          ...
          <label className="flex items-center gap-3 cursor-pointer bg-[#0F172A] border border-dashed border-[#334155] rounded-lg px-4 py-3 hover:border-indigo-500 transition-colors">
            ...
            <span className="text-slate-400 text-sm">{pdfName || 'Click to upload PDF...'}</span>
            <input
              type="file"
              accept=".pdf"
              className="hidden"
